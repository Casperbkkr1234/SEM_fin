...
class L_hat():
	...
	def Stopping_times(self, paths: torch.tensor,
		...
		for i in range(1, n_exercise_times):
			print(i)
			# reverse direction
			...
			stopping_times[:, :, j-1] = torch.where(g_n >= forward, j, stop_time)
		...
		h2 = g.squeeze(1)
		h3 = h2.numpy()
		g_0 = torch.take_along_dim(g.squeeze(1), stopping_times[:, :, 0], 1)
		h1 = g_0.numpy()
		m1 = g_0[:, 0]
	...
	def Tau(self, paths: torch.tensor,
		...
		exercise_steps = [int(date / dt) for date in exercise_times]
		for i in range(n_exercises):
			net = networks[i]

			if i != 0:
				forw = net.forward(paths[:, :, i])
				stopping_times[:, :, i] = torch.where(g[:, :, i] >= forw, i, g.shape[-1]-1)  # TODO fix 10**10
				a = g[:, :, i]
				k1 = a.numpy()
				...
				k9 = d.detach().numpy()
				reqw = 1
				stopping_times[:, :, i] = torch.where(g[:, :, i] >= networks[i], 10**10, g.shape[-1]-1)  # TODO fix 10**10
				ar=3
